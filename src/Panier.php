...
<div class="conteneur-produits">
    <?php if (!empty($_SESSION['panier'])) : ?>
        <?php foreach ($_SESSION['panier'] as $index => $produit) : ?>
            <div class="produit" id="produit-<?= $index ?>">
                <img src="<?= htmlspecialchars($produit['image']) ?>" alt="<?= htmlspecialchars($produit['nom']) ?>">
                <h2><?= htmlspecialchars($produit['nom']) ?></h2>
                <p class="prix">CA$<?= number_format($produit['prix'], 2) ?></p>

                <form method="post" action="Panier.php" class="form-supprimer">
                    <input type="hidden" name="supprimer" value="<?= $index ?>">
                    <button type="submit" class="bin-button supprimer-produit">
                        <!-- SVG du bouton de suppression -->
